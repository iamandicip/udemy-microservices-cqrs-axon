    ...
    public ResponseEntity<OpenAccountResponse> openAccount(@Valid @RequestBody OpenAccountCommand command) {
        ...
        try {
            ...
            return new ResponseEntity<>(
                    new OpenAccountResponse(id, "Successfully opened a new bank acount"),
                    HttpStatus.CREATED
            );
        } catch (Exception e) {
            var safeErrorMessage = "Error while processing request to open account for id: " + id;
            System.out.println(e);
            return new ResponseEntity<>(new OpenAccountResponse(id, safeErrorMessage), HttpStatus.BAD_REQUEST);
        }
    }
}
